...
array <int> control[trial_training/3];
control.fill(1,control.count(), 1,0);
array <int> left_trials[trial_training/3];
left_trials.fill(1,left_trials.count(),2,0);
array <int> right_trials[trial_training/3];
right_trials.fill(1,right_trials.count(),3,0);

trial_id.append(control);
trial_id.append(left_trials);
trial_id.append(right_trials);
trial_id.shuffle();
...
loop trial_counter=1 until trial_counter > trial_training # go through all trials for training
begin	
...
info1.set_caption("Training finished! \n\n Start the experiment?");
info1.redraw();
info1_trial.present();